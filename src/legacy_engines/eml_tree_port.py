...
def validate_expr_for_tree(node, depth, variable_names=("x", "y")):
    errors = []
    allowed_variables = set(variable_names)

    def _check(n, d):
        if isinstance(n, str):
            if n in allowed_variables and d != depth:
                errors.append(f"Terminal '{n}' at depth {d}, need {depth}")
            elif n != "1" and n not in allowed_variables:
                errors.append(f"Terminal '{n}' not available for this tree")
            return
        _check(n[1], d + 1)
        _check(n[2], d + 1)
...
def init_from_expr(tree, expr_str, k=32.0):
    ...
    errors = validate_expr_for_tree(parsed, td, variable_names=tree.input_names)
    if errors:
    ...
    choice_map = tree.leaf_symbol_to_idx

    def recurse(node, level, pos):
        if isinstance(node, str):
            if level == td:
                if node not in choice_map:
                    available = ", ".join(tree.leaf_choice_names)
                    raise ValueError(f"Terminal '{node}' not available. Choices: {available}")
                c = choice_map[node]
                with torch.no_grad():
                    tree.leaf_logits[pos] = torch.full((tree.n_leaf_choices,), -k, dtype=REAL_DTYPE)
...
def init_from_blend_leaves(tree, blend_str, leaf_str, k=32.0):
    ...
    for i, s in enumerate(symbols):
        if s not in choice_map:
            available = ", ".join(tree.leaf_choice_names)
            raise ValueError(f"Leaf {i}: unknown '{s}'. Use {available}.")
        leaf_choices.append(choice_map[s])

    with torch.no_grad():
        for i, c in enumerate(leaf_choices):
            tree.leaf_logits[i] = torch.full((tree.n_leaf_choices,), -k, dtype=REAL_DTYPE)
...
class EMLTree(nn.Module):
    ...
    def __init__(
        ...
        if len(self.input_names) not in (1, 2):
            raise ValueError("EMLTree supports one or two inputs.")
        self.leaf_choice_names = ("1",) + self.input_names
        self.leaf_symbol_to_idx = {name: idx for idx, name in enumerate(self.leaf_choice_names)}
        self.n_leaf_choices = len(self.leaf_choice_names)

        if init_strategy == "manual":
            leaf_init = torch.zeros(self.n_leaves, self.n_leaf_choices, dtype=REAL_DTYPE)
            gate_init = torch.zeros(self.n_internal, 2, dtype=REAL_DTYPE)
        elif init_strategy == "biased":
            leaf_init = torch.randn(self.n_leaves, self.n_leaf_choices, dtype=REAL_DTYPE) * init_scale
            leaf_init[:, 0] += 2.0
            ...
            leaf_init = torch.randn(self.n_leaves, self.n_leaf_choices, dtype=REAL_DTYPE) * init_scale
            if self.n_leaf_choices > 1:
                leaf_init[:, 1:] += 1.0
            gate_init = torch.randn(self.n_internal, 2, dtype=REAL_DTYPE) * init_scale + 4.0
        elif init_strategy == "random_hot":
            leaf_init = torch.randn(self.n_leaves, self.n_leaf_choices, dtype=REAL_DTYPE) * init_scale
            hot_idx = torch.randint(0, self.n_leaf_choices, (self.n_leaves,))
    ...
    def forward(self, x, y=None, tau_leaf=1.0, tau_gate=1.0):
        ...
        if len(self.input_names) == 2:
            if y is None:
                raise ValueError("This tree requires two inputs.")
            y = y.to(DTYPE)
    ...
    def _format_weights_mma(self, discretize=True, snap_threshold=0.0):
        ...
        if discretize:
            ...
            lw = np.zeros((self.n_leaves, self.n_leaf_choices))
            ...
            def fmt_leaf(w):
                ...
                return "{" + ", ".join(f"{v:.12f}" for v in w) + "}"
            ...
            leaf_parts = ["{" + ", ".join(f"{v:.12f}" for v in w) + "}" for w in lw]
    ...
    def export_mathematica(self, filename, discretize=True, comment="", snap_threshold=0.0):
        ...
        lines += [
            ...
            "",
            f"(* Leaf weights: {{{', '.join(self.leaf_choice_names)}}} for each leaf *)",
            leaf_line,
            "",
...
def make_harmonic_envelope_data(json_path, harmonic_index, fit_time_min=0.0):
    json_file = Path(json_path)
    if not json_file.exists():
        raise FileNotFoundError(f"Harmonic envelope file not found: {json_file}")

    records = json.loads(json_file.read_text(encoding="utf-8"))
    record = None
    for item in records:
        if int(item.get("harmonic_index", -1)) == harmonic_index:
            record = item
            break
    if record is None:
        raise ValueError(f"Harmonic index {harmonic_index} not found in {json_file}")

    x_vals = np.asarray(record["decay_times_norm"], dtype=np.float64)
    y_vals = np.asarray(record["decay_values_norm"], dtype=np.float64)
    if fit_time_min:
        x_vals = fit_time_min + (1.0 - fit_time_min) * x_vals

    valid = np.isfinite(x_vals) & np.isfinite(y_vals)
    x_vals = x_vals[valid]
    y_vals = y_vals[valid]
    print(
        f"Loaded harmonic H{harmonic_index}: {len(x_vals)} points, "
        f"target={record['target_freq_hz']:.3f} Hz, actual={record['actual_freq_hz']:.3f} Hz"
    )
    print(f"Using fit_time_min={fit_time_min}")
    x_tensor = torch.tensor(x_vals, dtype=REAL_DTYPE)
    t_tensor = torch.tensor(y_vals, dtype=DTYPE)
    return x_tensor, None, t_tensor, record
...
def compute_losses(
    ...
    max_leaf_unc = 1.0 - (1.0 / leaf_probs.shape[1])
    leaf_unc = torch.clamp((1.0 - leaf_max) / max(max_leaf_unc, 1e-12), 0.0, 1.0).pow(uncertainty_power)
...
def analyze_snap(tree, snap_threshold=0.01):
    ...
    names = {idx: name for idx, name in enumerate(tree.leaf_choice_names)}
    uncertain_leaves = []
    for i in range(tree.n_leaves):
        max_p = leaf_probs[i].max()
        max_idx = leaf_probs[i].argmax()
        if max_p < 1.0 - snap_threshold:
            probs_fmt = ", ".join(f"{p:.4f}" for p in leaf_probs[i])
...
def train_one_seed(seed, strategy, args, x_train, y_train, t_train, manual_init_fn=None):
    ...
    tree = EMLTree(
        ...
        input_names=("x", "y") if args.data_mode == "grid" else ("x",),
    )
...
def parse_args():
    ...
    # Data
    a("--data-mode", type=str, default="grid", choices=["grid", "harmonic-envelope"])
    ...
    a("--harmonic-json", type=str, default="")
    a("--harmonic-index", type=int, default=1)
    a("--harmonic-fit-time-min", type=float, default=0.0)
...
def main():
    ...
    if args.data_mode == "grid":
        ...
        target_fn = None
        target_desc = f"harmonic envelope H{args.harmonic_index}"
    ...
    try:
        ...
        if args.data_mode == "grid":
            ...
            if not args.harmonic_json:
                raise ValueError("--harmonic-json is required for --data-mode harmonic-envelope")
            x_train, y_train, t_train, data_meta = make_harmonic_envelope_data(
                args.harmonic_json,
                args.harmonic_index,
                fit_time_min=args.harmonic_fit_time_min,
            )
            x_gen, y_gen, t_gen = x_train, y_train, t_train
        ...
        print(f"target: {args.target_fn} = {target_desc}" if args.data_mode == "grid" else f"target: {target_desc}")
        ...
        if data_meta is not None:
            print(
                f"harmonic_index={data_meta['harmonic_index']} "
                f"actual_freq_hz={data_meta['actual_freq_hz']:.6f} "
                f"n_points={len(x_train)}"
            )
